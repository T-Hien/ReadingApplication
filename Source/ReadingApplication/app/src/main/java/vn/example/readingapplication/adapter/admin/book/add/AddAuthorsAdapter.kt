...
class AddAuthorsAdapter(
    private val itemList: MutableList<Author>,
    private val onItemUpdateListener2: AddBookActivity,
    private val onItemUpdateListener4: AddBookActivity

) : RecyclerView.Adapter<AddAuthorsAdapter.CardViewHolder>() {

    interface OnItemUpdateListener2 {
        fun onItemUpdated2(data: List<Int?>)
    }
    interface OnItemUpdateListener4 {
        fun onItemUpdated4(data: List<String?>)
    }
    ...
    override fun onBindViewHolder(holder: CardViewHolder, position: Int) {
        ...
        itemList.map { it.name}?.let { onItemUpdateListener4.onItemUpdated4(it) }
    }
